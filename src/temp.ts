...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function renderGraph () {
    ...
    var allNodesGraph = canvas.append('g')
      .attr('class', 'nodes')
...
canvas.append('text').text(centerIs).attr('x', '40').attr('y', '40')
    ...
    function tickActions() {
      nodes.attr('cx', d => d.x = Math.max(10, Math.min(canvasWidth - 10, d.x)))
          .attr('cy', d => d.y = Math.max(10, Math.min(canvasHeight - 10, d.y)));
      
      texts.attr('x', d => d.x = Math.max(10, Math.min(canvasWidth - 10, d.x)))
          .attr('y', d => d.y = Math.max(10, Math.min(canvasHeight - 10, d.y)));
